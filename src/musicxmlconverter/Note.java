...
public class Note implements Comparable<Note>
{
	...
	public int compareTo(Note oNote)
	{
		...
		else //same X, sort by pitch
		{
			...
			{
//				if(STEP2INTERVAL.get(this.getStep()) < STEP2INTERVAL.get(oNote.getStep()))   
				if (NAME2CHROMATIC.get(Character.toString(step)) < NAME2CHROMATIC.get(Character.toString(oNote.getStep())))
				{
					return BEFORE;
				}
				else if (NAME2CHROMATIC.get(Character.toString(step)) > NAME2CHROMATIC.get(Character.toString(oNote.getStep())))
				{
					return AFTER;
				}
